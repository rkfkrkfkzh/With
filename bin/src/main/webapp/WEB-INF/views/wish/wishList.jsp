...
<body>
	<h3>찜 목록 페이지</h3>
	<form id="w" action="${pageContext.request.contextPath }/wish/wishList"
		method="post">
			...
			<table class="table">
				...
				<c:forEach var="wish" items="${list}">
					<tr>
						...
						<td><a
							href="${pageContext.request.contextPath}/product/view?product_num=${wish.p.product_num}">${wish.p.product_title}</a>
						</td>
						<td>₩ <fmt:formatNumber pattern="#,###"
								value="${product.product_price}"></fmt:formatNumber>
						</td>
						<td>${wish.p.product_add_date}</td>
						<td>${wish.p.product_writer}</td>
						<td>${wish.p.product_hit}</td>
						<!-- 판매 상태에 대한 값을 보기 좋게 변경함 -->
						<td><c:choose>
								<c:when test="${wish.p.product_type == 0}">
                                                판매중
                                            </c:when>
								<c:otherwise>
                                                판매완료
